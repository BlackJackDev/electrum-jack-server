...
def create_config(filename=None):
    ...
    config.set('server', 'banner', 'Welcome to Electrum for BlackJack!')
...
def start_server(config):
    ...
    print_log("Starting Electrum for BlackJack server on", host)
...
def stop_server():
    ...
    server_proc.join()
    chain_proc.join()
    print_log("Electrum for BlackJack server stopped")
